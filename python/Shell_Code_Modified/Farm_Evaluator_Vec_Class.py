...
def getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, 
    ...
    #added by Naveen
    power_per_turbine=power.transpose()
    power_per_turbine=np.sum(power_per_turbine*wind_inst_freq,axis=1)
...
#     if turb_coords is None:
#         turb_coords   =  getTurbLoc(default_location+'turbine_loc_test.csv')
    
#     # Load the power curve
#     power_curve   =  loadPowerCurve(default_location+'power_curve.csv')
...
#     power_per_turbine,total_energy = getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, 
#                   n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t) 
#     #print('Total power produced by the wind farm is: ', "%.12f"%(AEP), 'GWh')
#     return(total_energy,power_per_turbine)


class Farm_Eval_mainClass: 
    turb_specs    =  {   
                     'Name': 'Anon Name',
                     'Vendor': 'Anon Vendor',
                     'Type': 'Anon Type',
    ...
    power_curve=None
    wind_inst_freq=None
    n_wind_instances=None
    cos_dir=None
    sin_dir=None
    wind_sped_stacked=None
    C_t=None
    
    def __init__(self, default_location='../../data/Shell_Hackathon_Dataset/',wind_inst_freq_file=None):
        # Load the power curve
        self.power_curve   =  loadPowerCurve(default_location+'power_curve.csv')
    ...
    def get_AEP_results(self,turb_coords=None):
        # Turbine x,y coordinates
        if turb_coords is None:
            turb_coords   =  getTurbLoc(default_location+'turbine_loc_test.csv')
        ...
        # this is just a check and the function does not quantifies the amount by 
        # which the constraints are violated if any. 
        checkConstraints(turb_coords, self.turb_diam)

        #print('Calculating AEP......')
        power_per_turbine,total_energy = getAEP(self.turb_rad, turb_coords, self.power_curve, self.wind_inst_freq, 
